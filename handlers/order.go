...
func handleSeckillOrder(w http.ResponseWriter, r *http.Request) {
    ...
    // 1. Redis预减库存
    success, err := PreReduceSeckillStock(rp, productID)
    if err != nil || !success {
    ...
    // 3. 发送到消息队列
    err = rabbitmq.PublishSeckillOrder(string(msgJSON))
    if err != nil {
        // 回滚Redis库存
        RollbackSeckillStock(rp, productID)
...
func startSeckillOrderConsumer() {
    for {
        msg := rabbitmq.ConsumeSeckillOrder()
        var orderMsg map[string]interface{}
        json.Unmarshal([]byte(msg), &orderMsg)
        
        productID := int(orderMsg["product_id"].(float64))
        
        // 数据库最终扣减
        err := FinalReduceSeckillStock(db, productID)
        if err != nil {
            log.Printf("秒杀订单处理失败: %v", err)
            // 可加入重试机制
            continue
        }
        ...
        _, err = InsertOrder(db, &Order{
            UserID:    int(orderMsg["user_id"].(float64)),
            ProductID: productID,
            // ...其他订单字段
        })
